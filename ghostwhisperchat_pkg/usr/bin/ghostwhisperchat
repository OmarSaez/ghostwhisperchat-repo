...
# --- ACTUALIZACIÓN DE SISTEMA (REPO APT) ---
...
def perform_system_update():
    ...
    safe_print(f"{Colors.M}[*] Ejecutando actualización de sistema...{Colors.E}")
    try:
        ...
        safe_print(f"{Colors.G}[✔] Actualización lista. Reiniciando...{Colors.E}")
        # Reiniciar el script automáticamente
        os.execv(sys.executable, ['python3'] + sys.argv)
        
    except subprocess.CalledProcessError:
        safe_print(f"{Colors.F}[X] Error en la actualización (¿Contraseña incorrecta?){Colors.E}")
    except Exception as e:
        safe_print(f"{Colors.F}[X] Error inesperado: {e}{Colors.E}")

def check_repo_update():
    """Verifica silenciosamente si el repo tiene una versión mayor"""
    ...
    try:
        ...
        if latest_ver > APP_VER_NUM:
            safe_print(f"{Colors.W}[!] Nueva versión disponible: v{latest_ver}{Colors.E}")
            ...
            raw_print(f"{Colors.G}¿Deseas actualizar ahora? se necesitaran permisos ROOT (sudo) [s/n]: {Colors.E}")
            ...
            if do_up.lower().startswith('s'):
                perform_system_update()
                 ...
                 safe_print(f"{Colors.W}[*] Actualización pospuesta.{Colors.E}")
