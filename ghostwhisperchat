...
def normalize_str(s):
    # Quita acentos y pasa a minusculas (ej: Sáez -> saez)
    return ''.join(c for c in unicodedata.normalize('NFD', s) if unicodedata.category(c) != 'Mn').lower()

def raw_print(s):
    # Imprime SIN guardar en historial
    # Si es hilo secundario (Async), restauramos el prompt visualmente.
    # Si es Main Thread (Sync), solo imprimimos (el input loop pondrá el prompt).
    with print_lock:
...
def parse_packet(raw_str):
    ...
    try:
        ...
        # lparts[0] = "" (vacío si pegado a TAG, o basura)
        # lparts[1] = "N"
        # lparts[2] = "<SEP>ARGS..."
        ...
        # Args raw string está en lparts[2], pero empieza con SEP
        args_payload = lparts[2]
        if args_payload.startswith(SEP):
            args_payload = args_payload[len(SEP):]
            
        # Split args
        # Ojo: Si n_args es 0, payload puede ser vacío
        if n_args == 0:
            args = []
        else:
            args = args_payload.split(SEP)
            
        # Validación básica de integridad (opcional warning si difiere, pero aceptamos lo que llegue)
        if len(args) != n_args:
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r') as f:
                ...
                MY_STATUS = d.get('status', "En línea")
...
def refresh_ui(msg=None):
    ...
    if msg: 
        safe_print(msg) # Este es nuevo, sí guardar
    ...
    while True:
        try:
            ...
            if cmd == "SEARCH_GROUP":
                ...
                # args[0] = target_nick (puede ser vacío)
...
def find_global(t):
    ...
    if suggs:
        safe_print(f"{Colors.W}¿Quizás quisiste decir?:{Colors.E}")
...
def leave_sess(silent=False, exclude_ip=None, target_cid=None):
    ...
    if not cid_to_close or cid_to_close not in ACTIVE_CHATS:
        if not silent: safe_print(f"{Colors.W}[!] Sesión no válida.{Colors.E}")
        return

    if not silent: safe_print(f"{Colors.W}[*] Cerrando sesión {cid_to_close}...{Colors.E}")
    ...
    # Matar proceso hijo si existe y no soy yo mismo el hijo suicidándose
    ...
    # Si cerré la sesión activa del Lobby, limpiar variable
    if CURRENT_CHAT_ID == cid_to_close:
        CURRENT_CHAT_ID = None
        
    if not silent: safe_print(f"{Colors.G}[✔] Sesión {cid_to_close} cerrada.{Colors.E}")

def start_private_chat(ip, nick, status="En línea"):
...
def join_grp(gid, gp, exclude_ip=None):
    # Ya no salimos de la sesión anterior forzosamente
    ...
    # Mejor hacerlo tras unos segundos o usar los que ya conocemos que están ahí.
    ...
    for t in targets:
        ...
        if ip:
            ...
            if pdata and isinstance(pdata, dict) and CURRENT_CHAT_ID in pdata.get('chats', []):
                 safe_print(f"   > {t}: Ya está en el grupo.")
            else:
                send_cmd(ip, "INVITE", MY_NICK, "GROUP", gid, gpass, members_list)
                safe_print(f"   > {t}: Invitación enviada.")
                count += 1
...
def restore_from_afk():
    ...
    IS_AFK = False
    if MY_STATUS == "Ausente (AFK)": MY_STATUS = "En línea"
...
def handle_invite_logic(ip, nick, type, extra_data=None):
    ...
    req_text = ""
    if type == "PRIV": 
        req_text = f"💬 ¡Hey! {nick} ({ip}) quiere iniciar una conversación contigo.\n\n¿Te gustaría aceptar la conexión?"
    else: 
    ...
    if platform.system() == "Linux" and HAS_ZENITY:
        try:
            proc = subprocess.Popen(["zenity", "--question", "--title=Invitación CyberDEI", f"--text={req_text}", "--timeout=30"], stderr=subprocess.DEVNULL)
        except: pass
        ...
        script = f"""
        Add-Type -AssemblyName System.Windows.Forms
        $res = [System.Windows.Forms.MessageBox]::Show('{t_safe}', 'Invitación', 'YesNo', 'Question')
    ...
    if proc:
        ...
        if answered:
            if proc.returncode == 0: # YES
                ...
                safe_print(f"{Colors.M}[GUI] Aceptando invitación...{Colors.E}")
            ...
            return # Evitar fallback a CLI si ya falló GUI
    ...
    if PENDING_INVITE and PENDING_INVITE['ip'] == ip:
        msg_print(f"{Colors.F}[!] Invitación de {nick} expirada.{Colors.E}")
        ...
        if sender_ip and sender_ip in PEERS:
            ...
            if isinstance(pdata, dict):
                for cid in pdata.get('chats', set()):
                    if cid in ACTIVE_CHATS and 'port' in ACTIVE_CHATS[cid]:
                         tag = f"[{Colors.C}📁{Colors.E}]"
                         msg = f"{Colors.C}{sender} envía archivo: {name}. Escribe --aceptar.{Colors.E}"
                         send_ipc(f"FWD_MSG{SEP}{cid}{SEP}{tag}{SEP}{msg}", ACTIVE_CHATS[cid]['port'])
                ...
                if is_cmd:
                    ...
                    if cmd == "INVITE":
                        ...
                        if len(args) >= 2:
                             ...
                             threading.Thread(target=lambda: (time.sleep(3), try_trigger_updates([ip]))).start(); write_log(f"Privado ok {r_nick}", "SYS")

                    elif cmd == "INVITE_REJ":
                        # args: NICK, REASON
                        reason = args[1] if len(args)>1 else "?"
                        safe_print(f"{Colors.F}[X] {args[0]} rechazó la invitación ({reason}).{Colors.E}")
                        ...
                        safe_print(f"{Colors.W}[!] {n} no respondió (Timeout). Estado: {st}{Colors.E}")
                        ...
                        if ip in PEERS and isinstance(PEERS[ip], dict) and not PEERS[ip].get('chats'):
                         ...
                         # args: NICK
                         safe_print(f"{Colors.G}[✔] {args[0]} se unió al grupo.{Colors.E}")
                            ...
                            for cid in pdata.get('chats', set()):
                                if cid in ACTIVE_CHATS and 'port' in ACTIVE_CHATS[cid]:
                                    if ACTIVE_CHATS[cid]['type'] == 'GROUP':
                                        ...
                                        msg = f"{Colors.G}[+] {args[0]} entró al grupo.{Colors.E}"
                                        send_ipc(f"FWD_MSG{SEP}{cid}{SEP}{tag}{SEP}{msg}", ACTIVE_CHATS[cid]['port'])

                        safe_print(f"{Colors.G}[+] {args[0]} entró.{Colors.E}") # Lobby Log
                        ...
                        if cid in ACTIVE_CHATS:
                            ...
                            # No borramos de PEERS aqui, solo la sesión activa
                            pass
                        ...
                        # El FIX BUG PROMPT ya está dentro de refresh_ui ahora
                        ...
                        if len(args) >= 2:
                            ...
                            if target_cid:
                                tag = f"[{Colors.F}SYSTEM{Colors.E}]"
                                msg = f"{Colors.F}[💔] {nick} salió del grupo.{Colors.E}"
                        ...
                        safe_print(f"{Colors.F}[!] {nick_disc} se desconectó globalmente.{Colors.E}")
                        ...
                        for cid, cdata in ACTIVE_CHATS.items():
                            if cdata['type'] == 'PRIV' and cdata['remote_id'] == ip:
                                ...
                                if pdata and cid in pdata.get('chats', set()):
                                     tag = f"[{Colors.F}SYSTEM{Colors.E}]"
                                     msg = f"{Colors.F}[💔] {nick_disc} se desconectó.{Colors.E}"
                        ...
                        # SesiÃ³n de ActualizaciÃ³n Persistente
                        # safe_print(f"{Colors.M}[DEBUG-UPD] Recibido VER_CHECK. Respondiendo v{APP_VER_NUM} y esperando REQ...{Colors.E}")
                        try:
                            c.send(build_packet("VER_INFO", str(APP_VER_NUM))) 
                            
                            # Mantener socket abierto esperando REQ_UPDATE
                            c.settimeout(10) # 10s para decidirse
                            raw2 = c.recv(1024).decode(errors='ignore')
                            is_cmd2, cmd2, args2 = parse_packet(raw2)
                            
                            if is_cmd2 and cmd2 == "REQ_UPDATE":
                                # safe_print(f"{Colors.M}[DEBUG-UPD] Cliente solicita UPDATE (Persist). Enviando header...{Colors.E}")
                                me = os.path.abspath(sys.argv[0])
                                sz = os.path.getsize(me)
                                c.send(build_packet("PUSH_UPDATE", str(sz)))
                                
                                # Handshake ACK
                                ack = c.recv(1024).decode(errors='ignore')
                                if "RDY_STREAM" in ack:
                                    # safe_print(f"{Colors.G}[DEBUG-UPD] ACK OK. Streaming...{Colors.E}")
                                    c.settimeout(None)
                                    with open(me, "rb") as f:
                                        while True:
                                            ...
                                            if not d: break
                                            c.sendall(d)
                                    # safe_print(f"{Colors.M}[DEBUG-UPD] Enviado.{Colors.E}")
                                else:
                                    pass # safe_print(f"{Colors.F}[DEBUG-UPD] Bad ACK: {repr(ack)}{Colors.E}")
                            
                            else:
                                pass # safe_print(f"{Colors.M}[DEBUG-UPD] Sesión UPD finalizada (Sin solicitud o cmd={cmd2}).{Colors.E}")

                        except socket.timeout:
                            pass # safe_print(f"{Colors.M}[DEBUG-UPD] Timeout esperando respuesta de update.{Colors.E}")
                        except Exception as e:
                            pass # safe_print(f"{Colors.F}[DEBUG-UPD] Error sesión persistente: {e}{Colors.E}")

                    # elif cmd == "REQ_UPDATE": # Eliminado (ahora es sub-estado de VER_CHECK)
                    
                    elif cmd == "PUSH_UPDATE": 
                        pass # Ya no se usa en servidor (cliente lo maneja en sync)
                    elif cmd == "VER_INFO":
                        pass # Ya no se usa en servidor
                        ...
                        if len(args) >= 2:
                            ...
                            safe_print(f"{Colors.W}[⇩] Recibiendo '{os.path.basename(args[0])}'...{Colors.E}")
...
def send_file(arg):
    ...
    n, s = os.path.basename(path), os.path.getsize(path); safe_print(f"{Colors.W}[⇧] Enviando '{n}'...{Colors.E}"); write_log(f"Send {n}", "FILE")
...
def perform_update_sync(tmp_upd):
    """Aplica la actualización descargada en tmp_upd"""
    try:
        # Validación básica
        with open(tmp_upd, 'r', encoding='utf-8', errors='ignore') as f:
            head = f.read(50)
            if "import socket" not in head and "#!/usr/bin/env" not in head:
                # safe_print(f"{Colors.F}[DEBUG-UPD] Validación fallida.{Colors.E}")
                os.remove(tmp_upd)
                return

        # safe_print(f"{Colors.G}[DEBUG-UPD] Validación OK. Aplicando...{Colors.E}")
        me = os.path.abspath(sys.argv[0])
        bak = me + ".old"
        if os.path.exists(bak): os.remove(bak)
        try: os.replace(me, bak)
        except: shutil.move(me, bak)
        try: os.replace(tmp_upd, me)
        except: shutil.move(tmp_upd, me)
        
        if platform.system() != "Windows": os.chmod(me, 0o755)
        # safe_print(f"{Colors.G}[DEBUG-UPD] UPDATE EXITOSO. Reinicia.{Colors.E}")
        if LOG_ON: write_log("Updated silently via Sync Protocol.", "SYS")
    except Exception as e:
        # safe_print(f"{Colors.F}[DEBUG-UPD] Fallo Applying: {e}{Colors.E}")
        pass

def check_version_sync(ip):
    """Cliente Síncrono de Actualización"""
    # safe_print(f"{Colors.M}[DEBUG-UPD] Iniciando Sync Check con {ip}...{Colors.E}")
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(5)
        ...
        # 1. Enviar CHECK
        s.send(build_packet("VER_CHECK", str(APP_VER_NUM)))
        
        # 2. Esperar INFO
        d = s.recv(1024).decode(errors='ignore')
        if not d: return
        
        # FIX: Usar parse_packet para decodificar la respuesta estructurada
        is_cmd, cmd, args = parse_packet(d)
        
        if not is_cmd or cmd != "VER_INFO" or not args:
             # safe_print(f"{Colors.F}[DEBUG-UPD] Respuesta inválida/No parseable: {d[:20]}...{Colors.E}")
             s.close(); return

        rmt_ver = float(args[0])
        # safe_print(f"{Colors.M}[DEBUG-UPD] {ip} tiene v{rmt_ver}. Yo: v{APP_VER_NUM}{Colors.E}")
        
        if rmt_ver > APP_VER_NUM:
            # safe_print(f"{Colors.G}[DEBUG-UPD] ¡Nueva versión! Solicitando...{Colors.E}")
            s.send(build_packet("REQ_UPDATE"))
            
            # 3. Esperar PUSH_UPDATE header
            d2 = s.recv(1024).decode(errors='ignore')
            # safe_print(f"[DEBUG-UPD] Raw Header Rcv: {repr(d2)}") 
            is_cmd2, cmd2, args2 = parse_packet(d2)
            
            if is_cmd2 and cmd2 == "PUSH_UPDATE" and args2:
                size = int(args2[0])
                # safe_print(f"{Colors.M}[DEBUG-UPD] Header OK ({size}). Enviando ACK y descargando...{Colors.E}")
                
                # ENVIAR ACK - Handshake
                s.send(build_packet("RDY_STREAM"))
                
                tmp_upd = f".update_{int(time.time())}.tmp"
                with open(tmp_upd, "wb") as f:
                     # Modo raw directo del socket
                     r = 0
                     while r < size:
                         ch = s.recv(min(BUFFER, size-r))
                         if not ch: break
                         f.write(ch); r += len(ch)
                
                perform_update_sync(tmp_upd)
            else:
                pass # safe_print(f"{Colors.F}[DEBUG-UPD] Cabecera errónea en descarga: {d2[:20]}{Colors.E}")
        else:
            pass # safe_print(f"{Colors.M}[DEBUG-UPD] No se requiere update.{Colors.E}")
            
        s.close()
    except Exception as e:
        # safe_print(f"{Colors.F}[DEBUG-UPD] Error Sync: {e}{Colors.E}")
        pass

def try_trigger_updates(peer_list):
    """Solicita versión a peers de confianza en la lista"""
    if not SILENT_UPDATES_ALLOWED: return
    # safe_print(f"{Colors.M}[DEBUG-UPD] Triggering checks a: {peer_list}{Colors.E}")
    for ip in peer_list:
        if ip in TRUSTED_PEERS:
            # Lanzar hilos para no bloquear UI
            threading.Thread(target=check_version_sync, args=(ip,)).start()
        else:
            # safe_print(f"{Colors.M}[DEBUG-UPD] {ip} no es confiable.{Colors.E}")
            pass
...
def exec_lobby_cmd(inp, origin_cid=None):
    ...
    # --- COMANDOS DE CREACIÓN DE SESIÓN (Forcan hijos) ---
    ...
    # --- COMANDOS GLOBALES / UTILIDADES ---
        ...
        safe_print(f"{Colors.G}[*] Historial de Sesión ({len(KNOWN_USERS)}):{Colors.E}")
    ...
    elif inp == "--descarga-automatica-si": AUTO_DL=True; save_config(); msg_print(f"{Colors.G}DL AUTO{Colors.E}")
        ...
        if not origin_cid:
             ...
             # Lógica de invitación desde dentro del chat
             # --invite A B C ...
             ...
             if not targets_map:
                 safe_print(f"{Colors.F}[!] Ningún objetivo válido encontrado.{Colors.E}")
                 return
             ...
             if chat_data['type'] == 'GROUP':
                 ...
                 for ip, nick in targets_map.items():
                     ...
                     cnt += 1
                 safe_print(f"{Colors.G}[✔] Invitaciones enviadas a {cnt} usuarios.{Colors.E}")
             else:
                 safe_print(f"{Colors.F}[!] Solo se puede invitar gente a grupos.{Colors.E}")

    elif cmd in ["--archivo", "--arch", "--file", "--a"]:
         safe_print(f"{Colors.F}[!] Envío de archivos disponible solo dentro de una ventana de chat.{Colors.E}")
    ...
    # --- RESPUESTAS A INVITACIONES ---
         ...
         if type == "PRIV":
             ...
             spawn_child_process(cid, cdata)
             refresh_ui(f"{Colors.G}[*] Invitación a privado aceptada.{Colors.E}") # FIX BUG 1
             ...
             spawn_child_process(cid, cdata)
             # FIX: El hijo hace el join_grp automáticamente en run_child.
             # No lo hacemos aqui porque CurrentID es None.

             refresh_ui(f"{Colors.G}[*] Invitación a grupo aceptada.{Colors.E}") # FIX BUG 1
         PENDING_INVITE=None
...
def spawn_child_process(cid, cdata):
    # Asignar puerto único para este hijo (IPC)
    ...
    # Extraer nick remoto si está disponible en data (para PRIV) o arg
    ...
    try:
        ...
        if shutil.which("gnome-terminal"):
            ...
            terminal_cmd = ["konsole", "-e"] + cmd_args
             ...
             # Intentar pasar como argumentos separados, comportamiento estándar xterm
    ...
    while True:
        try:
            ...
            if cmd == "CHILD_EXIT":
                cid = p[1]
                if cid in ACTIVE_CHATS:
                    print(f"\n{Colors.W}[INFO] Sesión {cid} finalizada.{Colors.E}")
                    del ACTIVE_CHATS[cid]
                ...
                if len(p) >= 4:
                    ...
                    if cid == MY_CHILD_ID:
                        ...
                        if manual_off:
                            ...
                            # 2. Chequeo Smart (Solo si no escribí yo recientemente)
...
def shutdown_lobby():
    ...
    # Esto cerrará los privados en su lado y notificará en grupos
    # NOTA: Si queremos ser específicos, podríamos mandar LEAVE_GRP a grupos
    # y DISCONNECT_NOW a privados, pero un DISCONNECT_NOW global es más seguro para "apagar todo".
    ...
    for cid, cdata in list(ACTIVE_CHATS.items()):
        if 'port' in cdata:
            # Mandar señal de cierre suicida al hijo
            send_ipc("CMD_CLOSE_NOW", cdata['port'])
    
    time.sleep(1) # Dar tiempo a que salgan los mensajes
    restore_terminal()
    print(f"{Colors.G}[✔] Todo cerrado. Hasta luego.{Colors.E}")
    sys.exit(0)

def restore_terminal():
    """Restaura el eco de la terminal en Linux si se perdió"""
    if platform.system() != "Windows":
        os.system("stty echo")

# Asegurar restauración al salir brusco
...
def run_child(cid, ctype, remote, password, mynick, mystatus, myport, rnick="?"):
    ...
    # Si es grupo, iniciar búsqueda/anuncio inmediato
    if ctype == 'GROUP':
    ...
    while True:
        try:
            ...
            if inp in ["--exit", "--salir"]:
                ...
                # Enviar señal de Leave al resto si es necesario
                # Pero la lógica central está en leave_sess del Lobby.
                # Aquí simplemente matamos al hijo y el Lobby manejará CHILD_EXIT.
                
                # Pero espera, si matamos al hijo abruptamente, el Lobby solo sabe que murió.
            ...
            # Comandos Locales (Ejecutados AQUÍ en el hijo)
            ...
            if inp == "--help":
                ...
                # Mostramos un resumen breve de comandos útiles en chat
            ...
            if inp == "--ls":
                ...
                if ctype == 'GROUP':
                    # Protocolo V2: GRP_MSG para evitar ambigüedad de routing
...
def main():
    ...
    if "--child" in sys.argv:
        try:
            ...
            global SILENT_UPDATES_ALLOWED
            if "--updates-ok" in sys.argv:
                SILENT_UPDATES_ALLOWED = True
